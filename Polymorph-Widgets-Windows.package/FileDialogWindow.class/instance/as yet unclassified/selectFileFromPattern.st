selectFileFromPattern
	...
	subMatches := matches select: [:de | de isDirectory not].
	subMatches size = 1 ifTrue: [
		^self selectedFileIndex: (f indexOf: subMatches first)].
	subMatches := matches select: [:de | de isDirectory].
	subMatches size = 1 ifTrue: [^self selectedFileIndex: (f indexOf: subMatches first)].
